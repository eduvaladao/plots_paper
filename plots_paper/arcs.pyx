...
cpdef double xmenos(double phi, double r0, double s, double phi0, double theta, double epsilon):
    ...
    """
    return 1 + (1/(1 - epsilon*Cos(2*(phi - phi0))))*(s*Cos(theta - phi) - epsilon*s*Cos(theta + phi - 2*phi0) - Sqrt((r0**2)*(1 - epsilon*Cos(2*(phi - phi0))) - (s**2)*(1 - epsilon**2)*(Sin(theta - phi)**2)))
